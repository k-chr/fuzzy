# -*- coding: utf-8 -*-
import numpy as np
from .ga.algorithm import optimize as ga_optimize
from .pso.algorithm import optimize as pso_optimize
...
class ANFIS:
	...
	def __init__(self, inputs, training_data: np.ndarray, expected_labels: np.ndarray, operator_function=TNORMS["Algebraic T-Norm"], operator_init_value=0.5):
	...
	def train(self, global_optimization: bool, learn_premises: bool, learn_operators: bool, learn_consequents: bool, n_iter=100, use_special=SpecialOptim.NONE, bounds_premises=None, **special_kwargs):
		...
		if learn_premises and learn_operators and learn_consequents:
			...
			bounds = bfv + bop + btsk
			if use_special is not SpecialOptim.NONE:
				d= {"fun_args":[self], "constr":bounds}
				special_kwargs.update(**d)
				if use_special is SpecialOptim.GA:
					res = ga_optimize(goal_premises_operators_consequents, minimize=True, num_of_iterations=n_iter, **special_kwargs)
				else:
					res = pso_optimize(goal_premises_operators_consequents, minimize=True, num_of_iterations=n_iter, dims = len(bounds), **special_kwargs)
