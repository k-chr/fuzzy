...
class FuzzyInputVariable_3Trapezoids:
    ...
    def showRegions(self, ax, orientation):
        ...
        if orientation==0:
            ax.add_patch(Rectangle((begin,self.c-self.w/2-self.fl),100,self.fl, fill=False, hatch='/',color='r'))
            ...
            ax.add_patch(Rectangle((self.c-self.w/2-self.fl,begin),self.fl,100, fill=False, hatch='/',color='b'))
...
class FuzzyInputVariable_List_Trapezoids:
    ...
    def __init__(self, listCWFLFR: List[List[float]], name: str, labels: List[str]):
        ...
        self.functionsList = listCWFLFR #type: List[float]
    ...
    def show(self, x=None):
        ...
        color_idx = 0
        for i in range(0, self.n_functions):
            plt.plot(x, y[:, i], label=f"{self.name}: {self.labels[i]}", color=colors[color_idx], linestyle=ls[color_idx])
            color_idx += 1
